//the starting place for this video is the line chart from prior
// video 2 for the line
// video 3 for the axis
// video 6 for the responsive behavior
...
// usePrevious to avoid infanent loops when react re renders
...
function BrushChart({ data }) {
  ...
  // making use of usePrevious to have the prevousSelection at desposal
  ...
  useEffect(() => {
    ...
    const yScale = scaleLinear()
      .domain([0, max(data)])
      .range([height, 0]);
    // line generator function from d3 path generator to use in svg
    const lineGenerator = line()
      .x((d, index) => xScale(index))
      .y((d) => yScale(d))
      .curve(curveCardinal);

    // render the line - path element
    svg
      .selectAll(".myLine")
      .data([data])
      .join("path")
    ...
    // brush logic brushX is from D3 define 3 things
    ...
    const brush = brushX()
      ...
      .on("start brush end", () => {
        ...
        if (event.selection) {
          ...
          // and for the circles so that they can be used bit differenlty if they are behind the brush or not.
          setSelection(indexSelection);
        }
      });

    // initial position + retaining position on resize
    // only if the prevous selection is the same as this selection call the code block
    // - this gets us out of the infanant loop
    if (previousSelection === selection) {
      svg
        ...
        // transforming index values  to pixel values  because selection or brush move needs pixels values
        // to avoid an infenent loop need to make sure that this code block
        // is only called when the selection is changing
